...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  try {
        ...
        $mail->SMTPAuth 	= true;                        				
        $mail->Host 		  = 'srv9.niagahoster.com';  // Masukkan Server SMTP
        $mail->Port 		  = 465;                                      // Masukkan Port SMTP
        $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
        $mail->Username 	= 'no-reply@pinopi.com';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
        $mail->Password 	= 'pinopi123';        						          // Masukkan Password dari Email tsb
        $default_email_from       = 'no-reply@pinopi.com';        // Masukkan default from pada email
        $default_email_from_name  = 'PINOPI.COM';           // Masukkan default nama dari from pada email
